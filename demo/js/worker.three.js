...
onmessage = function (e) {
    ...
    if (type === 'Polygon') {
        ...
        this.postMessage(result, [result.position, result.normal, result.uv]);
    }
};
...
function arrayBufferToVector3(buffer) {
    ...
    for (let i = 0, len = ps.length; i < len; i += 3) {
        ...
        vs.push(new THREE.Vector3(x, y, z));
    }
    return vs;
}
...
function generateExtrudePolygons(datas) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        const buffGeom = getExtrudeGeometry(datas[i]);
    ...
    return { position: position.array.buffer, normal: normal.array.buffer, uv: uv.array.buffer, faceMap, geometriesAttributes };

}

function getExtrudeGeometry(d) {
    const { data, height } = d;
    let shape = toShape(data);
