...
function throttle(fn, time, context) {
    var lock, args, _wrapperFn, later;

    later = function later() {
        lock = false;
        if (args) {
            _wrapperFn.apply(context, args);
            args = false;
        }
    };

    _wrapperFn = function wrapperFn() {
        if (lock && _wrapperFn.time > 0) {
            args = arguments;
        } else {
            fn.apply(context, arguments);
            setTimeout(later, _wrapperFn.time);
            lock = true;
        }
    };

    _wrapperFn.time = time;
...
function btoa(input) {
    ...
    for (var block, charCode, idx = 0, map = b64chrs, output = ''; str.charAt(idx | 0) || (map = '=', idx % 1); output += map.charAt(63 & block >> 8 - idx % 1 * 8)) {
...
var Map = function (_Handlerable) {
    ...
    Map.prototype.distanceToPixel = function distanceToPixel(xDist, yDist, zoom) {
        ...
        var p0 = this.coordinateToContainerPoint(center, zoom),
...
var CanvasRenderer = function (_Class) {
    ...
    CanvasRenderer.prototype.hitDetect = function hitDetect(point) {
        ...
        var detectPoint = map._pointToContainerPoint(point);
    ...
    CanvasRenderer.prototype.createCanvas = function createCanvas() {
        ...
        var map = this.getMap();
        var size = map.getSize();
        var r = Browser$1.retina ? Browser$1.pixelRatio : 1;
    ...
    CanvasRenderer.prototype.isRenderOnZooming = function isRenderOnZooming() {
        return false;
    };

    CanvasRenderer.prototype.isRenderOnMoving = function isRenderOnMoving() {
    ...
    CanvasRenderer.prototype.onPitch = function onPitch() {};

    CanvasRenderer.prototype.onRotate = function onRotate() {};
...
Map.include({
    ...
    _zoomTo: function _zoomTo(nextZoom, origin) {
        ...
        this._calcMatrices();
        var zoomOffset = this._getZoomCenterOffset(origin);
    ...
    _getZoomCenterOffset: function _getZoomCenterOffset(origin) {
        if (!origin) {
            return null;
        }

        var po = this._prjToContainerPoint(this._startZoomOrig);
...
Map.include({
    ...
    setPitch: function setPitch(pitch) {
        ...
        var p = clamp(pitch, 0, 60) / 180 * Math.PI;
    ...
    getCameraMatrix: function getCameraMatrix() {
    ...
    _pointToContainerPoint: function _pointToContainerPoint(point, zoom) {
        ...
        if (this._pixelMatrix) {
            ...
            transformMat4(t, t, this._pixelMatrix);
    ...
    _calcMatrices: function _calcMatrices() {
        ...
        var altitude = this._altitude = 1.5;

        var halfFov = Math.atan(0.5 / altitude);
        var topHalfSurfaceDistance = Math.sin(halfFov) * altitude / Math.sin(Math.PI / 2 - this._pitch - halfFov);

        var farZ = Math.cos(Math.PI / 2 - this._pitch) * topHalfSurfaceDistance + altitude;

        var m = new Float64Array(16);
        perspective(m, 2 * Math.atan(this.height / 2 / altitude), this.width / this.height, 0.1, farZ);
        translate(m, m, [0, 0, -altitude]);

        scale(m, m, [1, -1, 1 / this.height]);
        ...
        translate(m, m, [1, -1, 0]);
        this._pixelMatrix = multiply(new Float64Array(16), m, this._projMatrix);
        ...
        m = create();
        scale(m, m, [this.width / 2, -this.height / 2, 1]);
        this._cameraMatrix = multiply(m, m, m2);
    },
...
var CanvasLayerRenderer = function (_CanvasRenderer) {
    ...
    CanvasLayerRenderer.prototype.isRenderOnZooming = function isRenderOnZooming() {
        if (this.layer.options['renderOnZooming']) {
            return true;
        }
        return _CanvasRenderer.prototype.isRenderOnZooming.call(this);
    };

    CanvasLayerRenderer.prototype.isRenderOnMoving = function isRenderOnMoving() {
        if (this.layer.options['renderOnMoving']) {
            return true;
        }
        return _CanvasRenderer.prototype.isRenderOnMoving.call(this);
    };
...
var TileLayerDomRenderer = function (_Class) {
    ...
    TileLayerDomRenderer.prototype.render = function render() {
        var updateTiles = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
        ...
        var camMat = this.getMap().getCameraMatrix();
    ...
    TileLayerDomRenderer.prototype._getTileQueue = function _getTileQueue(tileGrid) {
        var tiles = tileGrid['tiles'],
            queue = [];
        var mat = this.getMap().getCameraMatrix();
        ...
        for (var _i = tiles.length - 1; _i >= 0; _i--) {
            cachedTile = this._tiles[tiles[_i]['id']];
            if (cachedTile) {
    ...
    TileLayerDomRenderer.prototype._prepareTileContainer = function _prepareTileContainer() {
        var map = this.getMap();
        var camMat = map.getCameraMatrix();
        var container = this._getTileContainer();
        if (!camMat) {
    ...
    TileLayerDomRenderer.prototype.onZooming = function onZooming(param) {
        ...
        if (this._levelContainers && this._levelContainers[zoom]) {
            ...
            if (map.getCameraMatrix()) {
                ...
                multiply(m, m, map.getCameraMatrix());
    ...
    TileLayerDomRenderer.prototype._getTileSize = function _getTileSize() {
        ...
        if (Browser$1.webkit && (map.getCameraMatrix() || map.isZooming())) {
            tileSize[0] += 2;
            tileSize[1] += 2;
        }
        return tileSize;
    ...
    TileLayerDomRenderer.prototype._onMapMoving = function _onMapMoving() {
...
var VectorLayerRenderer = function (_OverlayLayerCanvasRe) {
    ...
    VectorLayerRenderer.prototype.isRenderOnZooming = function isRenderOnZooming() {
        var map = this.getMap();
    ...
    VectorLayerRenderer.prototype._drawGeos = function _drawGeos() {
...
var MapCanvasRenderer = function (_MapRenderer) {
    ...
    MapCanvasRenderer.prototype._getCountOfGeosToDraw = function _getCountOfGeosToDraw() {
        ...
        for (var i = layers.length - 1; i >= 0; i--) {
            ...
            if (layers[i] instanceof OverlayLayer && layers[i].isVisible() && !layers[i].isEmpty() && (renderer._hasPointSymbolizer || map.getPitch())) {
    ...
    MapCanvasRenderer.prototype.initContainer = function initContainer() {
        ...
        var mapAllLayers = createContainer('allLayers', 'maptalks-all-layers', POSITION0 + 'padding:0px;margin:0px;', true);
        var front = createContainer('front', 'maptalks-front', POSITION0 + 'will-change:transform;', true);
        var frontLayer = createContainer('frontLayer', 'maptalks-front-layer', POSITION0);

        var ui = createContainer('ui', 'maptalks-ui', POSITION0 + 'border:none;z-index:10000;', true);
        var back = createContainer('back', 'maptalks-back', POSITION0 + 'will-change:transform;');
        var backLayer = createContainer('backLayer', 'maptalks-back-layer', POSITION0);
    ...
    MapCanvasRenderer.prototype._checkSize = function _checkSize() {
        ...
        this._resizeFrame = requestAnimFrame(function () {
            if (!_this2.map || _this2.map.isMoving() || _this2.map.isZooming()) {
                return;
            }
    ...
    MapCanvasRenderer.prototype._checkSizeLoop = function _checkSizeLoop() {
